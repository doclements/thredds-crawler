...
class ThreddsCrawler():
    ...
    def getTopLevel(self):
        ...
        self.catalog['title'] = self._getNode(
            xml_root, '/catalog:catalog/@name')[0]
        services = [(x['base'], x['name']) for x in [dict(x.items()) for x in self._getNode(
    ...
    def getSub(self, id):
        choice = [
            x for x in self.catalog['top_level'] if x['id'] is int(id)][0]
